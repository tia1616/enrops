...
\- \[task] Auto-add Enrops registrants to `marketing_recipients`. Surfaced during FA26 ship: J2S had 10 confirmed registrations through Enrops but only 7 of those parents were on the marketing list — the other 3 paid + enrolled but were never marketing-list-subscribed, so they're invisible to campaigns. Build path: on registration confirmation, upsert into `marketing_recipients` with `source = 'registration'`, default subscribed=true with an unsubscribe link in every email (already shipped). Operator opt-out config probably belongs on `organizations.registration_marketing_optin` (default true). Bonus: a Settings panel that surfaces "X registrants not on your marketing list" with a "Add them" button for the historic gap. (Cosmetic but related: Q2's "10 registered / 7 excluded" copy now explains the gap inline — fix landed before this task; see Q2_Who.jsx.)
