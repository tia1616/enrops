...
\- \[task] **Build the FA26 program/camp cancellation flow (one-button "Cancel this offering").** Asked by Jessica 2026-06-09 after cancelling **Forest Grove Wk 2** (SU26) entirely by hand via direct DB writes. Today there is NO operator-facing cancel path: `camp_sessions.status` has an `active`/`cancelled` CHECK but **nothing in the UI ever sets it**, and cancellation otherwise happens one registration at a time through the RefundDrawer. Builds on the broader 2026-05-26 "Cancel-a-program flow" item below (programs+camps unification) — fold these together, don't build twice. The flow must, on confirm: (1) set the session/program status → `cancelled`; (2) withdraw instructor assignments (`camp_assignments.status='withdrawn'`; programs need `program_assignments` equivalent) so the instructor frees up — and actually NOTIFY them (see [[project_enrops_instructor_notifications_gap]] — withdrawing is silent today); (3) refund active registrations (full / partial / keep-admin-fee per [[feedback_refund_admin_fee_pattern]]) — for FA26 this works in-app because native regs carry a real `stripe_payment_intent_id`, UNLIKE SU26 (see finding below); (4) stop/exclude the camp from marketing (see bug below); (5) notify the partner/venue with internal (non-"cancel"-to-parents) copy. **Why FA26 will actually be cleaner than SU26:** all 276 active SU26 camp regs have **no Stripe payment in Enrops** (Squarespace-origin) — Forest Grove Wk 2 had `current_enrollment=1` but ZERO registration rows, so the family is only refundable in Squarespace and isn't even visible in Enrops. FA26 native registration removes that whole class of manual reconciliation.

\- \[task] **BUG (found 2026-06-09): a cancelled camp is still promotable by marketing.** `marketing-touchpoint-send/index.ts` (~line 362) selects `camp_sessions` by `id IN (campaign camp_session_ids)` with **no `status <> 'cancelled'` filter**. So any campaign whose `camp_session_ids` includes a now-cancelled session will still render and pitch it to parents. Today this is latent (only **draft** campaigns reference Forest Grove Wk 2 — nothing scheduled/sending), but the moment a draft is launched it'd advertise a dead camp. Fix: exclude `status='cancelled'` in that query (and the ~line 1447 one) so cancelled sessions silently drop from any campaign. Small, do it with the cancellation-flow build. See [[project_enrops_camps_renderer_gap]].

\- \[task] **In-app preview for master-list (and person/auto) campaigns — no "send a test" required.** Logged 2026-06-09 while reviewing the Option-A preview-drawer reskin. The touchpoint email preview ("Preview as parent in…") only renders for **school/area-scoped** campaigns; a **master_list** blast intentionally leaves the dropdown disabled and the operator must use **"Send test to me"** to see the email (by design, documented in `ScheduleReview.jsx:80` + `:143`). That's exactly the gap [[feedback_in_app_preview]] flags — operators want a true in-app render, not send-to-self. Build (additive): a **generic in-app render** for master_list / person / auto scopes — the email rendered with sample/default token values — shown in the SAME new `EmailPreviewDrawer` (`src/pages/admin/marketing-v2/EmailPreviewDrawer.jsx`). Needs a no-location/sample render path in `marketing-touchpoint-send` (today preview mode requires a `preview_location_id`). Keeps the drawer UX consistent across every scope. Not a bug — the master-list case has no school/area to render "as a parent at X", so this is net-new functionality.

\- \[task] **Partners↔Locations unification ("a school is one thing") — BLOCKED on a data reconciliation; do AFTER Italy.** Decided 2026-06-09 to pull this merge forward, then pulled back after checking prod data. **The blocker:** `partners` and `program_locations` were populated from DIFFERENT sources and never reconciled. Prod has **100 partners** (66 public_school, 11 school_district, 10 parks_rec, 5 community_org/private_school, etc. — mostly a contact directory; **89 have NO venue**) vs **61 venues** (real camp locations, all with addresses; **50 have NO partner link**); only ~5 share an exact name and ~11 are actually linked via `partner_id`. A naive UI merge today would show ~150 rows, most flagged "no venue"/"no school", with the same school sometimes appearing twice — worse, not better. **Prerequisite (the real work, a DATA project not a UI task):** reconcile partners↔venues — fuzzy-match by name+address → set `partner_id` on the matches → dedupe → classify the partnerless directory orgs (districts, etc.). The deterministic partner-import + self-heal already exists ([[project_enrops_partners_locations_link]], shipped to staging 6/4) but the **prod backfill is still pending** — run/verify that FIRST. THEN the UI unify is easy + clean: one "Schools" list (partner+venue as a single row when linked), a single "Add a school" flow that creates partner + auto-linked venue, collapse the Partners/Locations tabs (Calendars stays its own tab); presentation-only, data model unchanged, so rosters/scheduling/marketing keep working. Note the Figma keeps Locations as a separate screen, so the 2026-06-09 reskin pass correctly leaves them as tabs (the new Locations edit-drawer stands as the interim). **Post-Italy.**

\- \[task] **enrops.com homepage — State 2 flip + `/login` redesign (follow-ups to State 1, shipped to prod 2026-06-09).** State 1 (invite-only holding card) is LIVE at `/` (`src/pages/enrops/Landing.jsx`) per Arielle's brief (`C:\Users\JVorster\Downloads\enrops_com_homepage_spec.md`). Two follow-ups: (1) **State 2 (self-serve), when Arielle says go** — copy swap on the SAME card: drop the "Founding cohort" chip, H1→"Log in or create your account.", remove body, secondary button → "Create a free account" linking `/signup`, add "New to enrops? Learn more →" → getenrops.com. BLOCKED until self-serve operator signup + the tenant onboarding wizard exist (no `/signup` route today; we're invite-only/white-glove). (2) **`/login` page redesign** — Jessica deferred ("login is fine"). Today `/login` renders the existing universal sign-in (cream card, Google + password + magic-link) — works, but Arielle's brief specs a deep-purple card matching the homepage + "Forgot password?" + "Don't have an account?" links. Low priority, do when convenient. Also note: marketing "Start Free" CTAs land on the holding card (waitlist), not a real free signup, until State 2 — Arielle may relabel to "Get Early Access" for the invite-only phase (her call). Tiny cleanup: the disabled `dev-signin-link` staging edge fn can be fully deleted from the Supabase dashboard.

\## 2026-06-08

\- \[task] **No direct parent/family messaging surface — blocks the "B6: Parent Communication, Handled" onboarding Loom as written.** Surfaced 2026-06-08 while planning the B-series Looms. The B6 script promises an operational comms hub: message ONE family (billing question, absence), message ONE program's enrolled roster (schedule/weather change), and a sent-message HISTORY/"who-received-it-when" log. None of that exists as a distinct surface. What's live is **Family Comms → Campaigns** (the 4-question AI campaign builder, `Q2_Who.jsx`), which CAN target one person / one school-location / the master list — but every send routes through the **marketing** pipeline (Resend + promotional unsubscribe footer), so it's the wrong tool for transactional/operational notes, and there's no confirmed per-message history view. Build (post-alpha, scoped): a real parent-messaging surface for operational sends — (1) recipient = one family OR one program's enrolled roster OR full list, (2) sends as **transactional** (bypasses the promotional unsubscribe — depends on the transactional/promotional split already in this backlog under 2026-06-03), (3) a sent log per recipient group (delivered/opened/when). Until it exists, re-record B6 around campaign audience-targeting only, drop the 1:1-billing-note and message-history beats. Note: the B2 "Plan Your Season" Loom is also affected — its hero beat ("enrops suggests assignments based on availability + match") is the not-yet-shipped auto-matcher (see the FA26 instructor-schedule-wizard item under 2026-06-03); film manual-assign + send-offers instead. See [[project_enrops_marketing_standing_rules]].

\- \[✅ RESOLVED 2026-06-08 — see [[project_enrops_operator_stripe_activation]]] **RESOLUTION:** Shipped `sync-operator-stripe-status` (operator status-refresh via the **v1 Accounts API** `accounts.retrieve`, which is account-shape-agnostic — returns charges/payouts for v1 AND v2 accounts) to **prod + staging**. `Finances.jsx` calls it on `?stripe=return` AND via a manual "Already finished? Check status" button. Chose the refresh-fallback over a v2 webhook handler (deterministic, no stripe-node SDK bump, mirrors the instructor `refresh-stripe-status` pattern). Staging-tested (Cascade onboarding→active) + prod-verified (Netlify commit_ref + edge-fn ACTIVE on both). Corrected framing: a stuck operator doesn't "lose registrations" — `create-checkout` falls back to a direct charge, so funds mis-route to Enrops's platform account; the fn writes both `stripe_charges_enabled` and `stripe_account_status` to fix it. *(Original entry retained below for the audit trail.)* **Operator Stripe activation may break for v2 Connect accounts.** Surfaced 2026-06-08. Our operator-activation webhook (`stripe-webhook` → `handleAccountUpdated`) only processes the classic **v1 `account.updated`** event. But the *current* `stripe-connect-onboard` code, when run in the Enrops Dev sandbox, produced a **v2 account** (`related_object.type = v2.core.account`, emitting `v2.core.account.*` events) that did NOT trigger the v1 handler — even with the endpoint subscribed to v1 `account.updated`. Prod J2S activated fine on v1, but that may be because the LIVE platform still defaults to v1 accounts (sandboxes often run ahead) or J2S predates Stripe's v2 default. **Risk:** if the live platform now mints v2 accounts for NEW operators, a fresh alpha founder finishes Stripe onboarding but Enrops never flips them to `active` → registrations never open. There is NO frontend status-refresh fallback for operators (only the webhook updates `stripe_account_status`; `sync-operator-stripe-status` exists on prod but isn't called from the UI). **Robust fix (build next session, scoped, money-path-careful):** add a `v2.core.account.*` handler to `stripe-webhook` so activation works for v1 AND v2 accounts; test on staging (v2 events already flow there). Alternatively/also: confirm what a fresh LIVE-platform account emits. Do NOT consider operator self-serve Stripe connect fully proven for arbitrary new tenants until this is closed.

\- \[✅ RESOLVED 2026-06-08 — see [[project_enrops_operator_stripe_activation]]] **RESOLUTION:** `sync-operator-stripe-status` is now in the repo and deployed to **both** staging and prod (identical), so a staging operator reaches `active` via the UI ("Check status" button or `?stripe=return`). We did NOT wire a staging Connect webhook — the refresh-fallback is account-shape-agnostic so the v1-vs-v2 event mismatch is moot for activation. *(Original entry retained below.)* **Staging is missing the operator Stripe-status sync (parity gap).** Found 2026-06-08 while testing #16 refunds. After an operator finishes Stripe Connect onboarding on staging, `organizations.stripe_account_status` stays `onboarding` (charges/payouts false) forever — no `account.updated` Connect webhook reaches staging (`stripe_last_account_event_id` is null) AND the operator status-sync edge fn (`sync-operator-stripe-status`, present on PROD v6) is **not in the repo and not deployed to staging**. So no staging tenant can reach "active" via the UI. Workaround used: manually set `stripe_account_status='active'` + charges/payouts true on the J2S staging org (left connected per Jessica 2026-06-08). Fix: (1) get `sync-operator-stripe-status` into the repo and deploy to staging; (2) wire the Connect webhook on staging; (3) re-verify after. **2026-06-08 finding:** attempted the staging webhook wiring and hit a **v1-vs-v2 Stripe events mismatch** — the Enrops Dev *sandbox* surfaces/routes **v2 `v2.core.account.*`** events, but our handler (`stripe-webhook` → `handleAccountUpdated`) listens for the classic **v1 `account.updated`**. The endpoint created in the new Workbench subscribed to v2 events, so the v1 handler never fired. PROD is unaffected (J2S has `stripe_last_account_event_id` set = a v1 `account.updated` was processed; account-creation code pins `apiVersion 2023-10-16` = v1 accounts). To finish staging: create the webhook as a **v1 / snapshot endpoint subscribed to `account.updated`** (matching prod), not a v2 event destination. `STRIPE_WEBHOOK_SECRET_CONNECT` is already set on staging (from the Enrops Dev sandbox). Until then, operator Stripe-connect can't be self-serve-tested on staging — but the prod alpha path is proven. See [[feedback_absolute_parity_staging_prod]].

\- \[task] **Finances money-dashboard: account search → account detail → refund-from-payment (fast-follow to #16).** Logged 2026-06-08. The #16 pass ships the Rosters refund action + a read-only refund ledger in Finances. The richer money-context surface (Stripe/Square pattern) is deferred: search a family by child/parent name or email → open their account → list their payments (program/camp, amount, date, refunds already issued) → Refund button per payment that reuses the SAME shared RefundDrawer component built in #16. No new edge fn (refund-registration already covers it). Pre-alpha if time allows; otherwise post-alpha. See [[feedback_refund_admin_fee_pattern]].

\- \[task] **Enrops-branded refund/withdrawal confirmation email to the parent (deferred from #16 refund build).** Logged 2026-06-08. When an operator refunds/withdraws a registration via the new Rosters refund drawer, the parent currently only gets **Stripe's automatic refund email** — Enrops sends nothing. For alpha that's acceptable (decision 2026-06-08). Post-alpha: send an Enrops-branded "your refund/withdrawal is confirmed" email (per-tenant branding, amount, program, whether the spot was kept or freed), and consider an in-app parent notification once the parent notifications surface (#18) exists. Keep it gated to real Enrops refund events. See [[feedback_refund_admin_fee_pattern]].

\- \[task] **Offline roster imports bypass the intelligence-layer doorway.** Surfaced 2026-06-08 during the roster-import overhaul. `admin-import-camp-roster` and `admin-import-program-roster` insert `registrations` rows directly (service-role) but never call `public.log_enrollment_event()`, so operator-uploaded / offline enrollments are invisible to the sealed `intelligence` schema — unlike native checkout (create-checkout `initiated` + stripe-webhook `payment_completed`). These are real enrollments the moat should capture. Fix: emit an enrollment event per inserted registration via the doorway (no-PII, source tagged as operator-import, e.g. `roster_imported`). See [[project_enrops_intelligence_layer]] + `docs/moat/INTELLIGENCE_LAYER_RULES.md`.
...
\- \[task] **Build + test "Enrops moves the money" instructor payout (the `enrops_platform` pay route).** Logged 2026-06-08. Today ONLY J2S's own-platform route (`legacy_own_platform`) can pay instructors via Stripe — proven by the live $1 test. The `enrops_platform` route (operator's Enrops-connected account balance -> instructor Express account) is DRAFTED but deliberately blocked: `pay-instructor` returns 501 `pay_route_not_yet_supported` so untested code never moves real money. KEY POINT: an operator connecting Stripe for **registrations** does NOT unlock payroll — paying out to instructors is a separate outbound flow that needs (1) the instructor-side Express payout onboarding wired for the enrops_platform model (instructor signs up name/SSN/bank under Enrops's platform the first time they're paid — operators connecting their own account does nothing for their instructors), and (2) the operator-connected-account -> instructor-Express transfer path finished + tested end-to-end with a non-J2S tenant. THEN (3) expand the Payroll "Pay via Stripe" gate (`Payroll.jsx` `canStripePayout`, added 2026-06-08) to include enrops_platform tenants with charges enabled — today it's gated to `legacy_own_platform` only, so manual / enrops_platform tenants correctly see "Mark paid manually" only. Until built, new tenants pay instructors manually. Money always comes from the OPERATOR's Stripe balance, never Enrops's. Post-alpha (manual pay is fine for the hand-onboarded founding cohort). See [[project_enrops_pay_scheme]].
...
\- \[cleanup] **Custom term picker to replace the native 26px `<select>` on both schedule headers (camp + after-school).** The term switcher is a `<select>` styled as a 26px heading; its native dropdown renders huge/awkward options. Replace with a custom dropdown (button trigger + menu) used by both the camp `HeaderStrip` and the after-school `Header` so they're consistent and not "big and weird." (After-school trigger reduced to 20px as a stopgap.)

\- \[task] **After-school staffing view — remaining parity nice-to-haves (post Stage B):** day-of sub coverage (assignment_substitutions is already program-aware), Print schedule route for after-school, the Instructor Hat tips, drag-drop assign, and realtime board updates when an instructor responds. Stage B (the offer loop) is the prerequisite for most of these.

\- \[task] **Upload-a-schedule for providers NOT running registration on Enrops (after-school).** The after-school schedule is registration-driven — programs (times, schools, curriculum) and enrollment all come from Enrops reg. A provider whose registration is off Enrops has no native programs, so the after-school term can't populate. Build an upload/import path (PDF / doc / CSV of their schedule -> parsed into `programs` for the term). The new-cycle modal already promises this ("Not running registration on Enrops? You'll be able to upload your schedule instead") — until built, off-Enrops-reg providers can't use after-school scheduling. Native reg is the happy path; this is the fallback.

\- \[later] **Camps converge to the same registration-driven model as after-school.** Eventually camp scheduling (`camp_sessions`) should also rely on Enrops registration instead of a manually-dated cycle + derived weeks. J2S dogfooded SU26 manually this summer while the platform was being built. When camps move to reg-driven, the "+ New cycle" camp path simplifies the same way after-school just did (no manual dates/weeks — pulled from reg). Future convergence; not now.
