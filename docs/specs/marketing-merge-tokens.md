...
| `{{registration_close_date}}` | `programs.first_session_date - organizations.registration_close_days_before`, formatted as `"Monday, August 24"` | 🟢 | Afterschool only. Emitted only when ALL of the recipient's picked programs share one close date (same rule as camps' `early_bird_deadline`), because `{{curriculum}}` names them all in the same sentence. Empty for camps, for a missing start date, and when a school's programs disagree — use `{{program_details}}` there. Describes operator practice — nothing closes checkout automatically. |
| `{{program_details}}` | HTML `<ul>`, one `<li>` per program at the recipient's school, sorted by start date | 🟢 | Each row carries its OWN day, start date, session count and sign-up deadline, so a multi-program school is described correctly instead of one program's facts being applied to all of them. Renders `""` when empty, so it can never leave a dangling half-sentence the way an inline token can. Own line only, never inside a sentence. Afterschool sibling of `{{camp_details}}`; empty for camps. |
