...
-- program_rosters / instructors_read_roster_student_contacts). A program SUB had
-- no roster access at all, so SubDetailView hid the roster for program subs even
-- though two sub emails promise it. This mirrors the CAMP sub model exactly:
...
GRANT EXECUTE ON FUNCTION public.sub_roster_program_ids() TO authenticated, anon;
