...
import { getCaller, assertRole } from '../_shared/assertRole.ts';
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ---- Authenticate caller BEFORE any work (no existence-probing pre-auth). ----
    // Without this, ANY authenticated user (incl a parent or viewer) could trigger
    // a real send. verify_jwt only proves a valid token, not the right to send.
    const callerResult = await getCaller(req, supabase);
    if ('error' in callerResult) return callerResult.error;
    const callerAuthId = callerResult.caller.authUserId;
    ...
    // ---- Authorize: caller must be able to edit this org (owner/admin/staff). ----
    const roleCheck = await assertRole(supabase, callerAuthId, orgId, 'edit');
