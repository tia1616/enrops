  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      if (!confirmErr) {
        ...
        // ONE CHILD, ONE SEAT. Clear up the registration a family abandoned before
        // coming back and paying.
        //
        // Found on Jessica's staging walk (2026-08-21): click the invite, abandon Stripe,
        // return through the same link, pay. That leaves TWO pending rows for one child
        // and registration_holds_seat counts both, so the class read 7 of 6 until the
        // older one aged out 24 hours later.
        //
        // Reads FULLER, never emptier, so it can never oversell - the cost is that a
        // genuinely freed seat stays invisible to the waiting-list sweep for a day.
        //
        // AFTER the payment rather than at creation, deliberately: at creation nobody has
        // paid and both checkout sessions are live, so cancelling one means cancelling a
        // session the family might still be completing. Here the paid cart is known.
        //
        // Order against the consume above does not matter - the RPC only ever cancels
        // rows NOT in regIds, and the claim points at a row that IS in regIds - but it
        // runs second so that a failure here can never stop a family's place being spent.
        try {
          const { data: tidied, error: tidyErr } = await admin
            .rpc('cancel_superseded_pending_registrations', { p_registration_ids: regIds });
          if (tidyErr) {
            console.error('[stripe-webhook] could not cancel superseded pending registrations; a class may read one seat fuller than it is for 24h', {
              registration_ids: regIds, error: tidyErr.message,
            });
          } else if ((tidied ?? 0) > 0) {
            console.log('[stripe-webhook] cancelled an abandoned registration this family superseded', {
              registration_ids: regIds, rows_cancelled: tidied,
            });
          }
        } catch (e) {
          console.error('[stripe-webhook] cancel_superseded_pending_registrations threw', (e as Error).message);
        }
      }
      ...
      let orgSlug = '';
      // The fee columns ride along on the SAME read rather than costing a
      // second one. They are only used to work out what paying by bank saved
      // this family - see the discount below.
      let orgFeeRow: Record<string, unknown> | null = null;
      if (orgId) {
        const { data: orgSlugData } = await admin
          .from('organizations')
          .select('slug, fee_pass_through, platform_fee_card_pct, platform_fee_ach_pct, platform_fee_cap_cents, platform_fee_ach_cap_cents, platform_fee_floor_cents, platform_fee_override_until')
          .eq('id', orgId)
          .single();
        orgSlug = (orgSlugData?.slug as string) ?? '';
        orgFeeRow = (orgSlugData as Record<string, unknown>) ?? null;
      }
      if (!orgSlug) {
      ...
      if (parentEmail && regs?.length) {
        ...
        // WHAT PAYING BY BANK SAVED THEM. Money layer section 4 lists this on
        // the receipt, and it is NOT hypothetical: three organisations on
        // production charge 3% on card and 1% on bank today, so their families
        // genuinely pay less this way and the receipt should say so.
        //
        // create-checkout builds SINGLE-METHOD sessions, so payment_method_types
        // names the rail this family actually used rather than what was
        // offered. Only a bank payment can have saved anything; a card payment
        // paid the listed price and has no discount to show.
        //
        // The saving is the card fee MINUS what they were charged, both on the
        // same registrations - never two independently computed numbers. If the
        // org's two rates are equal, which is J2S and Jeff today, this is zero
        // and no line renders.
        let bankDiscountCents = 0;
        const paidByBank = Array.isArray(session.payment_method_types)
          && session.payment_method_types.length === 1
          && session.payment_method_types[0] === 'us_bank_account';
        if (paidByBank && enropsFeeCents > 0 && orgFeeRow?.fee_pass_through) {
          try {
            const feeCfg = resolveFeeConfig(
              orgFeeRow as unknown as Parameters<typeof resolveFeeConfig>[0],
              await loadPlatformFeeDefaults(admin),
            );
            const cardFee = cartFeeCents(
              (regs || []).map((r: { id: string; amount_cents?: number }) => ({
                registrationId: r.id,
            ...
            bankDiscountCents = Math.max(0, cardFee - enropsFeeCents);
          } catch (discountErr) {
            // A receipt must still send. Worst case the family simply does not
            // see what they saved, which is strictly better than no receipt.
            console.warn(`[stripe-webhook] could not compute the bank discount for session ${session.id}:`, discountErr);
          }
        }
...
async function sendConfirmationEmail({
  ...
  donationCents = 0, donationCoveredFeeCents = 0, enropsFeeCents = 0, bankDiscountCents = 0,
}: {
  admin: SupabaseClient;
  brand: OrgBrand;
  ...
  /**
   * What paying by bank saved this family, if they paid that way. 0 renders no
   * line - including when the org charges the same on both rails.
   */
  ...
  // THE FEE ROW SHOWS THE CARD FEE WHEN A DISCOUNT IS SHOWN, and this is the
  // only arrangement that both matches the doc and still adds up.
  //
  // A family paying by bank was charged the BANK fee, so "fee $4.80" followed
  // by "discount -$2.40" would make the rows sum to less than Total paid - the
  // very defect this receipt work exists to remove, reintroduced by the line
  // meant to be generous. Caught before landing by asking whether the receipt
  // still reconciles, which is the invariant, not a nice-to-have.
  //
  // So the fee row states the card fee, the discount takes it down to what
  // they actually paid, and registrations + fee - discount + gift = total.
  // That is also exactly section 4's column order: "Program price, enrops
  // service fee, bank payment discount if used, total paid".
  const feeRowCents = enropsFeeCents + bankDiscountCents;

  // Presented as a saving they already received, never as a card penalty - the
  // same framing rule the checkout uses, and the reason it is legal where a
  // surcharge is not.
  const bankDiscountRow = bankDiscountCents > 0
    ? `<tr><td style="padding:16px;border-bottom:1px solid #EDE9FE;font-family:${brand.font_family};">
          <div style="font-size:16px;font-weight:700;color:#1A1530;">Bank payment discount</div>
          <div style="font-size:14px;color:#6b6880;margin-top:4px;">You saved this by paying from your bank account instead of a card.</div>
        </td>
        <td style="padding:16px;text-align:right;vertical-align:top;border-bottom:1px solid #EDE9FE;font-family:${brand.font_family};font-weight:700;color:#1A1530;">
          &minus;${fmt(bankDiscountCents)}
        </td>
      </tr>`
    : '';

  const feeRow = feeRowCents > 0
    ? `<tr><td style="padding:16px;border-bottom:1px solid #EDE9FE;font-family:${brand.font_family};">
