...
export default function AddSchoolModal({ org, districts = [], partners = [], onClose, onCreated, onDistrictsChanged }) {
  ...
  async function resolveDistrictId() {
    ...
    if (districtId === "") {
      throw new Error(`Choose this ${venueWord(org)}'s district — or pick “No district” if it doesn't follow one.`);
    }
  ...
  async function save() {
    ...
    if (!trimmed) { setError(`${VenueWord(org)} name is required.`); return; }
    ...
    >
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
          <h2 style={{ margin: 0, fontSize: 19, fontWeight: 700, color: INK }}>{addVenueLabel(org)}</h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ fontSize: 12, color: districtId === NO_DISTRICT ? "#8a6d1f" : MUTED, marginTop: -6, lineHeight: 1.5 }}>
            {districtId === NO_DISTRICT
              ? `No district means no school calendar here, so this ${venueWord(org)}'s class dates won't skip no-school days. You can set a district later.`
              : districtId === ""
                ? `The district's calendar is what makes class dates skip no-school days. Pick the one this ${venueWord(org)} follows.`
